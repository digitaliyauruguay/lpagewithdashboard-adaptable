...
import { 
  ...
  Home, ShieldCheck, Activity, UserCheck
} from 'lucide-react';
...
// Configuración dual para veterinaria/peluquería
const businessConfig = {
  veterinary: {
    name: "VetCare Pro",
    hero: {
      title: "Cuidado Profesional para tu Mascota",
      subtitle: "Servicios veterinarios de excelencia con tecnología avanzada y amor por los animales"
    },
    services: [
      {
  ...
  grooming: {
    name: "Glamour Paws",
    hero: {
      title: "Estilo y Cuidado para tu Mascota",
      subtitle: "Servicios de peluquería canina y felina con productos premium y atención personalizada"
    },
    services: [
      {
        name: "Peluquería Completa",
        description: "Corte, baño y peinado profesional para todas las razas",
        icon: Scissors,
        color: "from-pink-600 to-pink-800",
        features: ["Corte", "Baño", "Peinado", "Limpieza"]
      },
      {
        name: "Spa y Relajación",
        description: "Tratamientos de bienestar y relajación para mascotas",
        icon: HeartHandshake,
        color: "from-purple-600 to-purple-800",
        features: ["Masajes", "Aromaterapia", "Hidroterapia", "Relajación"]
      },
      {
        name: "Estilismo Avanzado",
        description: "Cortes de moda y diseño personalizado",
        icon: Sparkles,
        color: "from-yellow-600 to-yellow-800",
        features: ["Diseño", "Tinte", "Accesorios", "Fotografía"]
      }
    ],
    stats: [
      { label: "Mascotas Estilizadas", value: 3000, suffix: "+" },
      { label: "Estilos Únicos", value: 50, suffix: "+" },
      { label: "Clientes Felices", value: 99, suffix: "%" }
    ]
  }
};

// Navegación enterprise con toggle de negocio
const BusinessNav = ({ isOpen, setIsOpen, currentSection, currentBusiness, setCurrentBusiness, scrollY }) => {
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Logo y nombre de empresa con toggle */}
          <div className="flex items-center gap-4">
            ...
            >
              ...
              >
                {businessConfig[currentBusiness].name}
              </motion.span>
            </div>
            ...
            <div className="flex items-center bg-gray-100 rounded-full p-1">
              <button
                onClick={() => setCurrentBusiness('veterinary')}
                className={`px-3 py-1 rounded-full text-sm font-medium transition-all ${
                  currentBusiness === 'veterinary' 
              ...
              <button
                onClick={() => setCurrentBusiness('grooming')}
                className={`px-3 py-1 rounded-full text-sm font-medium transition-all ${
                  currentBusiness === 'grooming' 
...
// Hero section dual
const DualHero = ({ currentBusiness }) => {
  const heroRef = useRef(null);
  const isInView = useInView(heroRef, { once: true });
  const { scrollYProgress } = useScrollEffects();
  const config = businessConfig[currentBusiness];
      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen">
        ...
        >
          ...
          >
            ...
            {currentBusiness === 'veterinary' ? 'Salud Animal' : 'Estilo Canino'}
          </motion.div>
...
// Servicios dual
const DualServices = ({ currentBusiness }) => {
        ...
        >
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            {currentBusiness === 'veterinary' 
              ...
              : 'Estilo y cuidado profesional para que tu mascota luzca increíble'
            }
          </p>
        </motion.div>
...
// Testimonios dual
const DualTestimonials = ({ currentBusiness }) => {
  const testimonialsRef = useRef(null);
  const isInView = useInView(testimonialsRef, { once: true });

  const testimonials = currentBusiness === 'veterinary' ? [
    {
      name: "María González",
      role: "Dueña de Max",
  ...
  ] : [
    {
      name: "Sofía López",
      role: "Dueña de Bella",
      content: "Mi poodle nunca se vio tan hermosa. ¡El corte es perfecto!",
      rating: 5,
      company: "Cliente Glamour"
    },
    {
      name: "Diego Silva",
      role: "Dueño de Thor",
      content: "Excelente servicio de spa para mi golden. Quedó relajado y brillante.",
      rating: 5,
      company: "Cliente Spa"
    },
    {
      name: "Lucía Pérez",
      role: "Dueña de Chanel",
      content: "Siempre innovando con nuevos estilos. Mi yorkshire luce increíble.",
      rating: 5,
      company: "Cliente Fashion"
    }
  ];
    ...
    <section id="testimonios" className="py-20 bg-gradient-to-b from-white to-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Historias de éxito de {currentBusiness === 'veterinary' ? 'dueños de mascotas saludables' : 'mascotas estilizadas'}
          </p>
        </motion.div>
...
// Contacto dual con validación avanzada
const DualContact = ({ currentBusiness }) => {
  ...
  const services = currentBusiness === 'veterinary' ? [
    'Consulta Médica',
    'Vacunación',
    'Cirugía',
    'Emergencia',
        ...
        >
          ...
          <p className="text-xl text-white/90 max-w-3xl mx-auto">
            {currentBusiness === 'veterinary' 
              ...
              : 'Estilo y belleza para tu mejor amigo'
            }
          </p>
        </motion.div>
...
export default function VetGroomingLanding() {
  const [isOpen, setIsOpen] = useState(false);
  const [currentSection, setCurrentSection] = useState('inicio');
  const [currentBusiness, setCurrentBusiness] = useState('veterinary');
  const { scrollY } = useScroll();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <DualHero currentBusiness={currentBusiness} />
      <DualServices currentBusiness={currentBusiness} />
      <DualTestimonials currentBusiness={currentBusiness} />
      <DualContact currentBusiness={currentBusiness} />
